    ...
    (() => {
        ...
        function initSliders() {
            if (document.querySelector(".swiper")) {
                ...
                if (window.innerWidth < 768) new swiper_core_Swiper(".countries__slider", {
                    modules: [ Navigation ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: "auto",
            ...
            if (containerPC) {
                ...
                if (checkboxes.length > 0) checkboxes.forEach((checkbox => {
                    checkbox.addEventListener("change", (() => {
                        ...
                        if (checkedCount >= maxChecked) checkboxes.forEach((cb => {
                            if (!cb.checked) {
                                cb.classList.add("disabled");
                                cb.disabled = true;
                                const nextElement = cb.nextElementSibling;
                                if (nextElement) nextElement.classList.add("disabled");
                            }
                        })); else checkboxes.forEach((cb => {
                            cb.classList.remove("disabled");
                            cb.disabled = false;
                            const nextElement = cb.nextElementSibling;
                            if (nextElement) nextElement.classList.remove("disabled");
                        }));
                    }));
                }));
            }
